...
public class CartService {
    @Autowired
    private PaymentCTConnector travelBuddiesConnector;

    public Product getProductByKey(String productKey) {
        Product product = travelBuddiesConnector.getDefaultPaymentApi().products().withKey(productKey).get().executeBlocking()
                .getBody();
        return product;
    }
    ...
    public CompletableFuture<Cart> addTripToCartAnonymous(Product product) {
        ...
        return travelBuddiesConnector.getDefaultPaymentApi().carts()
                .post(cartDraft)
